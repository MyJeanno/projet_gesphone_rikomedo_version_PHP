...
if(isset($_GET['id'])){
	...
	$row=$enc->getOne($id);
	$i=$row['ID_ENC'];
	$client=$enc->showNomClient($row['NUM_CLI']);
	$mont=$row['MONT_ENC'];
	$date=$row['DATE_ENC'];
	$numCli=$row['NUM_CLI'];
	//var_dump($client);
...
if(isset($_POST['suppr'])){
	$code=$_POST['code'];
	$montant=$enc->showOldSomme($code);
	$num=$enc->showNumClient($code);
	$client=$enc->showNomClient($num);
	//$message="L'utilisateur a supprimé un encaissement";
    $enc->delete($code);
    $cli->updateSoldeClidelEncaisse($montant,$num);
    $enc->updateSoldeEncaisseDel($montant,$num,$code);
    //$enc->AjouterSuppression($_SESSION['id'],date("Y-m-d H:i"),$message,$client);
	...
	<div class="jumbotron" style="background-color: #191970"> 	
	  <div class="container">
	   <hr>
		...
		<div class="span4"></div>
		<div class="span8">
			<h1 style="color: white;">Supprimer un encaissement</h1>
		</div>
	   </div>	
      <hr>
	  </div>
	 ...
	 <div class="row-fluid sortable">
		...
		<div class="box span8">
			<div class="box-header" style="background-color: #778899;" data-original-title>
				<h2><i class="halflings-icon edit"></i><span class="break"></span>Suppression encaissement</h2>
				<div class="box-icon">
			...
			<div class="box-content">
				<form class="form-horizontal" method="POST" action="DeleteEncaisse.php">
						  <fieldset>
						  ...
						  <h2 style="color:red; ">Etes-vous sûr de vouloir supprimer cet encaissement?</h2>
						  <table>
							  	<tr>
							  		<td><b>Id : </b></td>
