...
if(isset($_GET['id'])){
	...
	$nom=$opp->showNomActionnaire($IdAction);
	$prenom=$opp->showPrenomActionnaire($IdAction);
	$debut=$row['DATE_DEBUT'];
	$fin=$row['DATE_FIN'];
	$jour=$row['NBRE_JOUR'];
	$mont=$row['MONTANT_OPP'];
	$taux=$row['TAUX'];
}
...
if(isset($_POST['code'])){
	$code=$_POST['code'];
	$dateReelle=$_POST['date2'];
	$date1 = new DateTime($dateReelle);
	$date2 = $date1->format('Y-m-d');
    $nbre=$_POST['nbre'];
	$taux=$_POST['taux'];
	$commission=$_POST['comm'];	
	
    $opp->setIdOpportunite($code);
    $opp->setDateCloture($date2);
    $opp->setNbreJour($nbre);
    $opp->setTauxOpportunite($taux);
    $opp->setCommission($commission);
    $opp->setEtatOpportunite("Calculé");
    $test=$opp->updateOpportunite();
    
    header('location:ListeOppCalculer.php');	
}
	...
	<div class="jumbotron" style="background-color: #191970"> 	
	  <div class="container">
	   <hr>
		...
		<div class="span3"></div>
		<div class="span9">
			<h1 style="color: white;">Calcul des commissions d'une opportunité</h1>
		</div>
	   </div>	
      <hr>
	  </div>
	 ...
	 <div class="row-fluid sortable">
		...
		<div class="box span8">
			...
			<div class="box-content">
				<form class="form-horizontal" name="opportunite" method="POST" action="FormClotureOpp.php" style="background-color: #B0E0E6">
				  <fieldset>
	 	             ...
	 	             <div class="span6">
						...
						<div class="controls">
						  <input type="hidden" name="nj" value="<?php echo $jour;?>">
						</div>
				    </div>
	 	               <div class="control-group info">
                        ...
                        <div class="control-group info">
					       <label class="control-label">Solde opportunité</label>
							<div class="controls">
						  <input type="text" name="montant" readonly="true" value="<?php echo $mont;?>">
						</div>
				     </div>
				      <div class="control-group info">
						<label class="control-label">Date début</label>
						<div class="controls">
						  <input type="date"  name="date1" readonly="true" required="true" value="<?php echo $debut;?>">
						</div>
				      </div>
				      <div class="control-group info">
	 	             ...
	 	             <div class="span6">
				       ...
				       <div class="control-group info">
						  ...
						  <div class="controls">
						    <input type="text" name="taux" readonly="true" required="true" value="<?php echo $taux;?>">
						</div>
				        </div>				      
				       <div class="control-group info">
					  <label class="control-label" for="date01">Date de fin réelle</label>
					   <div class="controls">
				    ...
				    <div class="form-actions">
					  ...
					  <a class="btn btn-info" href="ListeOpportunite.php">Retour à la liste</a>
					</div>
					</fieldset>
					</form>   
					</div>
		...
		<script>
			...
			function calculerCommission(){
					...
					var nj=document.forms["opportunite"].elements["nj"].value;
					var tj = taux/nj;
