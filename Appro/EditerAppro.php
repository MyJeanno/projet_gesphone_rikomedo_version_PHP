...
if(isset($_POST['send'])){
	...
	$qtebdd=$app->showQteAppro($code);
	$idProd=$app->showIdProduit($code);
	$qteDiff=$quantite-$qtebdd;
    $message="Approvisionnement modifié";
	$elt=$prod->showLibelleProduit($idProd);

	$elt2=$prod->showLibelleProduit($produit);

	//var_dump($elt);
	//exit();

    $app->setIdAppro($code);
    $app->setQteAppro($quantite);
    $app->setDateAppro($date);  
    $app->updateApprovi();
    $prod->updateStockProduit($qteDiff,$idProd);
    //$prod->AjouterSuppression($_SESSION['id'],date("Y-m-d H:i"),$message,$elt);
    $prod->AjouterSuppression($_SESSION['id'],date("Y-m-d H:i"),$elt,$qtebdd,$elt2,$quantite,$message,"NEANT",$ori);
    header('location:ListAppro.php');
  }
?>
	...
	<div class="jumbotron" style="background-color: #191970"> 	
	  <div class="container">
	   <hr>
		...
		<div class="span4"></div>
		<div class="span8">
			<h1 style="color: white;">Editer un approvisionnement</h1>
		</div>
	   </div>	
      <hr>
	  </div>
	 ...
	 <div class="row-fluid sortable">
		...
		<div class="box span8">
			<div class="box-header" style="background-color: #778899;" data-original-title>
				<h2><i class="halflings-icon edit"></i><span class="break"></span>Formulaire approvisionnement</h2>
				<div class="box-icon">
			...
			<div class="box-content">
				<form class="form-horizontal" method="POST" action="EditerAppro.php" style="background-color: #B0E0E6">
				  <fieldset>
				    ...
				    <div class="control-group info">
					    <label class="control-label">Quantité</label>
					       <div class="controls">
					        <input type="number" required="true" name="qtte" value="<?php echo $qte;?>">
					       </div>
				    </div>
				    <div class="control-group info">
				    ...
				    <div class="form-actions">
					  ...
					  <a class="btn btn-info" href="ListAppro.php">Retour à la liste</a>
					</div>
					</fieldset>
					</form>   
					</div>
